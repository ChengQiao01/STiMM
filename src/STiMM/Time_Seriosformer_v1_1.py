...
from scripts.activations_hf import ACT2FN
...
class Seis_Time_Transformer(ViTPreTrainedModel):
    def __init__(self, config: TimeSeriesFormeConfig) -> None:
        ...
        # self.decoder = nn.Linear(config.hidden_size//config.patch_size, config.trace_num)
        ...
        self.upsample = UpsampleOneStep(scale=config.patch_size, num_feat=config.patch_size, num_out_ch=1)
    ...
    def forward(
        ...
        # reconstructed_pixel_values = reconstructed_pixel_values.unsqueeze(1)
        # reconstructed_pixel_values = reconstructed_pixel_values.permute(0, 1, 3, 2)

        masked_im_loss = None
        if bool_masked_pos is not None:
            bool_masked_pos = bool_masked_pos.reshape(-1, self.config.time_num//self.config.patch_size, 1)

            mask = bool_masked_pos.repeat_interleave(self.config.patch_size, dim=1).repeat(1, 1, self.config.trace_num).unsqueeze(1)  # [B, 1, T, X]
